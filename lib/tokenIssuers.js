...
const RefreshTokenIssuer = {
  * issueToken(client){
    ...
    const [refreshToken] = yield RefreshTokens
      ...
      .include(Tokens.select('id', 'clientId', 'createdAt'))
    ...
    if (refreshToken.token.clientId !== client.id || refreshToken.revoked || diff > conf.value('server.tokenExpiration')) {
    ...
    yield [
      Tokens.update()
        .set('revoked', true)
        .where('id', '$tokenId')
        .run({tokenId: refreshToken.tokenId}),
      refreshToken.save({revoked: true})
    ];
...
const AuthorizationCodeTokenIssuer = {
  * issueToken (client) {
    const {AuthorizationCodes, Tokens, RefreshTokens}=this.ctx.app.context;
...
module.exports = function (koaCtx) {
  ...
  switch (grant_type) {
    case 'authorization_code':
      return Object.create(Object.assign(AuthorizationCodeTokenIssuer, BasicClientAuthenticator), {ctx: {value: koaCtx}});
    case 'client_credentials':
      return Object.create(Object.assign(ClientCredentialsTokenIssuer, BasicClientAuthenticator), {ctx: {value: koaCtx}});
    case 'refresh_token':
      return Object.create(Object.assign(RefreshTokenIssuer, BasicClientAuthenticator), {ctx: {value: koaCtx}});
    default:
